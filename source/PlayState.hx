...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Z', 0.06],
		['You suck lmao', 0.3],
		['It\'s not overcharted, you\'re just bad', 0.5],
		['G', 0.65],
		['F', 0.69],
		['Nice', 0.7],
		['E', 0.75],
	...
	override public function create()
	{
		...
		healthBarBG = new AttachedSprite('healthBar' + ClientPrefs.healthBarType);
		...
		var songWatermark = new FlxText(4, textYPos, 0,
			SONG.song + " - " + storyDifficultyText	+ " - Psych Forever v4.1.1 (PE 0.5.2h)", 12);
		...
		if(curSong.toLowerCase() == "senpai" && ClientPrefs.shaders){
		...
		if(curSong.toLowerCase() == "roses" && ClientPrefs.shaders){
			addShaderToCamera('camGame', new VCRDistortionEffect(0.1, true, true, true));
			addShaderToCamera('camHUD', new VCRDistortionEffect(0.1, true, true, true));
		}
		if(curSong.toLowerCase() == "thorns" && ClientPrefs.shaders){
			addShaderToCamera('camGame', new VCRDistortionEffect(0.1, true, true, true));
			addShaderToCamera('camHUD', new VCRDistortionEffect(0.1, true, true, true));
		}
  ...
  public function clearShaderFromCamera(cam:String){
	...
	override public function update(elapsed:Float)
	{
		...
		if (ClientPrefs.scoretextstyle == 'New') {
	/*	if(ratingName == '?')
			scoreTxt.text = 'Score: ' + songScore + ' | Accuracy: 0%' + ' | Combo Breaks: ' + songMisses + ' (' + (songMisses + shits) + ')' + ' | Rank: F | N/A';
		else if (songMisses >= 65)
			scoreTxt.text = 'Score: ' + songScore + ' | Accuracy: ' + Highscore.floorDecimal(ratingPercent * 100, 0) + '% | Combo Breaks: ' + songMisses +' (' + (songMisses + shits) + ')' + ' | Rank: F' + ' | Skill Issue (ur trash lol)';
		else
			scoreTxt.text = 'Score: ' + songScore + ' | Accuracy: ' + Highscore.floorDecimal(ratingPercent * 100, 0) + '% | Combo Breaks: ' + songMisses + ' (' + (songMisses + shits) + ')' + ' | Rank: ' + ratingName + ratingFC;//peeps wanted no integer rating*/
		if(ratingName == '?') {
			scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' (' + (songMisses + shits) + ')' + ' | Accuracy: 0% (?)';
		}
		else {
		if (songMisses >= ClientPrefs.skillissuelevel && songMisses <= 500) {
			scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' (' + (songMisses + shits) + ')' + ' | Accuracy: ' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%' + ratingFC;
		}
		else if (songMisses >= 500) {
			scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' (' + (songMisses + shits) + ')' + ' | Accuracy: ' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%' + ratingFC + ' (WTF?)';
		}
		else {
		//	scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' (' + (songMisses + shits) + ')' + ' | Accuracy: ' + Highscore.floorDecimal(ratingPercent * 100, 2) + '% (Rank: ' + ratingName + ' | ' + ratingFC;
			scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' (' + (songMisses + shits) + ')' + ' | Accuracy: ' + Highscore.floorDecimal(ratingPercent * 100, 2) + '% (' + ratingName + ')' + ratingFC;
		}
		}
		}
		if (ClientPrefs.scoretextstyle == 'New Alt') {
			if(ratingName == '?') {
				scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' (' + (songMisses + shits) + ')' + ' | Rank: F (0%)';
			}
			else {
			if (songMisses >= 65 && songMisses <= 500) {
				scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' (' + (songMisses + shits) + ')' + ' | Rank: Skill Issue (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)';
			}
			else if (songMisses >= 500) {
				scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' (' + (songMisses + shits) + ')' + ' | Rank: What the fuck? (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)';
			}
			else {
				scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' (' + (songMisses + shits) + ')' + ' | Rank: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)';
			}
		}
		}
		if (ClientPrefs.scoretextstyle == 'Kade Engine') {
			if(ratingName == '?')
				scoreTxt.text = 'Score: ' + songScore + ' | Accuracy: 0%' + ' | Misses: ' + songMisses + ' (' + (songMisses + shits) + ')';
			if (songMisses == 0)
				scoreTxt.text = 'Score: ' + songScore + ' | Accuracy: ' + Highscore.floorDecimal(ratingPercent * 100, 2) + '% | Misses: ' + songMisses +' (' + (songMisses + shits) + ')' + ' | FC';
			if (songMisses == 0 && ratingPercent <= 0.95)
				scoreTxt.text = 'Score: ' + songScore + ' | Accuracy: ' + Highscore.floorDecimal(ratingPercent * 100, 2) + '% | Misses: ' + songMisses +' (' + (songMisses + shits) + ')' + ' | A';
			if (songMisses >= ClientPrefs.skillissuelevel)
				scoreTxt.text = 'Score: ' + songScore + ' | Accuracy: ' + Highscore.floorDecimal(ratingPercent * 100, 2) + '% | Misses: ' + songMisses +' (' + (songMisses + shits) + ')' + ' | Skill Issue (ur trash lol)';
			else
				scoreTxt.text = 'Score: ' + songScore + ' | Accuracy: ' + Highscore.floorDecimal(ratingPercent * 100, 2) + '% | Combo Breaks: ' + songMisses + ' (' + (songMisses + shits) + ')';//peeps wanted no integer rating
			}
		if (ClientPrefs.scoretextstyle == 'Psych') {
			if(ratingName == '?')
				scoreTxt.text = 'Score: ' + songScore + ' | Misses: ' + songMisses + ' (' + (songMisses + shits) + ')' + ' | Rating: ? (0%)';
			else if (songMisses >= ClientPrefs.skillissuelevel)
				scoreTxt.text = 'Score: ' + songScore + ' | Misses: ' + songMisses + ' (' + (songMisses + shits) + ')' + ' | Rating: ' + 'You Suck!' + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' - Skill Issue';
			else
				scoreTxt.text = 'Score: ' + songScore + ' | Misses: ' + songMisses + ' (' + (songMisses + shits) + ')' + ' | Rating: ' + (ratingPercent == 1 ? 'Perfect!!' : ratingPercent >= 0.9 ? 'Sick!' : ratingPercent >= 0.8 ? 'Great' : ratingPercent >= 0.7 ? 'Good' : ratingPercent == 0.69 ? 'Nice' : ratingPercent >= 0.6 ? 'Meh' : ratingPercent >= 0.5 ? 'Bruh' : ratingPercent >= 0.4 ? 'Bad' : ratingPercent >= 0.2 ? 'Shit' : ratingPercent >= 0 ? 'You Suck!' : "") + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ratingFC;
		}
		if (ClientPrefs.scoretextstyle == 'Score Only') {
		...
		if (health > 2)
			...
			if (ClientPrefs.improvedicons == 'Advanced') {
				if (healthBar.percent > 80) {
					iconP1.animation.curAnim.curFrame = 4; // victorus
					iconP2.animation.curAnim.curFrame = 0;
					scoreTxt.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.CYAN, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				  }
				 ...
				 if (healthBar.percent < 20 && healthBar.percent > 0) {
					...
					iconP2.animation.curAnim.curFrame = 4; // victorus
				 ...
				 if (healthBar.percent > 40 && healthBar.percent < 60) {
					iconP1.animation.curAnim.curFrame = 2; // healthy
					iconP2.animation.curAnim.curFrame = 2; // healthy
					scoreTxt.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				  }
				}
			else {
					if (healthBar.percent < 20) {
						iconP1.animation.curAnim.curFrame = 1; // You Losing
						scoreTxt.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.RED, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
						iconP2.animation.curAnim.curFrame = 2; // Oppoent Winning
					}
					else if (healthBar.percent > 80) {
						iconP1.animation.curAnim.curFrame = 2; // You Winning
						iconP2.animation.curAnim.curFrame = 1; // Oppoent Losing
						scoreTxt.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.CYAN, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
					}
					else {
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['week1_nomiss', 'week2_nomiss', 'week3_nomiss', 'week4_nomiss',
				...
				'ur_good', 'hype', 'two_keys', 'toastie', 'debugger', 'ass']);
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		switch (daRating)
		{
			case "shit": // shit
				...
				if (ClientPrefs.healthtype == 'Old') {
					...
					if (ClientPrefs.healthtype == 'New') {
					if (!ClientPrefs.latedamage)
						health += 0.03 * healthGain;
						else
						health -= 0.05 * healthLoss;
					}
			case "bad": // bad
		...
		// Don't call me a dumbass with the late ratings, I know there is probably a easier and faster way :(
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		if (!cpuControlled && !paused && key > -1 && (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || ClientPrefs.controllerMode))
		{
			if(!boyfriend.stunned && generatedMusic && !endingSong)
			{
				...
				else if (canMiss && ClientPrefs.antimashlol) {
					noteMissPress(key);
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (ClientPrefs.healthtype == 'Psych') {
				health += 0.03 * healthGain;
			}
	...
	override function stepHit()
	{
		...
		health += (ClientPrefs.suplove / 5000);
		health -= (ClientPrefs.poisonfright / 5000);
	...
	public function RecalculateRating() {
		...
		if(ret != FunkinLua.Function_Stop)
		{
			...
			ratingFC = "";
			if (ClientPrefs.scoretextstyle == 'New') {
			if (sicks > 0) ratingFC = " | (MFC) Sick!";
			if (goods > 0) ratingFC = " | (GFC) Great";
			if (bads > 0 ) ratingFC = " | (FC) Great";
			if (shits > 0) ratingFC = " | (FC-) Good";
			if (songMisses > 0 && songMisses < 10) ratingFC = " | (SDCB) Good";
			if (songMisses >= 10 && songMisses <= ClientPrefs.skillissuelevel) ratingFC = " | (Clear) Okay";
			else if (songMisses >= ClientPrefs.skillissuelevel) ratingFC = " | (Clear - Skill Issue) Skill Issue"; // Yeah I know, this is pretty much unused since it changes the score text entirelly.
			}
			else {
			...
			if (songMisses >= 10 && songMisses <= ClientPrefs.skillissuelevel) ratingFC = " - Clear";
			else if (songMisses >= ClientPrefs.skillissuelevel) ratingFC = " - Skill Issue"; // Yeah I know, this is pretty much unused since it changes the score text entirelly.
			}
		}
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				switch(achievementName)
				{
					case 'week1_nomiss' | 'week2_nomiss' | 'week3_nomiss' | 'week4_nomiss' | 'week5_nomiss' | 'week6_nomiss' | 'week7_nomiss':
						...
						{
							if (CoolUtil.difficultyString() == 'HARD' || CoolUtil.difficultyString() == 'HELL') {
